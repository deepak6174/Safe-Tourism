...
export default props => {
  ...
  const [locations] = useState(["Nahargarh Fort","Hawa Mahal","Amer Fort","Jantar Mantar","Jal Mahal","Jaigarh Fort", "Bhangarh Fort","Achrol Fort","City Palace","Rambagh Palace", "Birla Mandir","Galtaji Temple","Govindji Temple","Bhuteshwar Nath Mahadev","Sisodia Rani Garden","Raj Mandir Cinema"]);
    ...
    if (selection.time && selection.date) {
      (async _ => {
        ...
        let tables = res.tables.filter(
          table =>
            (selection.size > 0 ? table.seat >= selection.size : true) &&
      ...
      <Row noGutters className="text-center align-items-center pizza-cta">
        <Col>
          <p className="looking-for-pizza">
            {!selection.table.id ? "Register to visit" : "Confirm Reservation"}
            <i
              className={
                !selection.table.id
          ...
          <p className="selected-table">
            {selection.table.id
              ? "Looking for place to visit " + selection.table.name
              : null}
          </p>
            ...
            <Col>
              ...
              {selection.date && selection.time ? (
                getEmptyTables() > 0 ? (
                  ...
                  <p className="table-display-message">No Available Place to Visit</p>
                )
              ) : (
